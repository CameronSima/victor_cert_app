...
def sendmail(sender, recipient, certified):
	mail.send_mail(sender=sender,
				   ...
				   body='A new certificate has been purchased and awarded to ' + certified +'.' )
...
class PaymentHandler(Handler):
	def get(self, id):
			# a = db.Key.from_path('Priestessess', int(id))
			# post = db.get(a)
			# cert_name = post.name
...
class PDFHandler(Handler):
	...
	def post(self):
		name = self.request.get('name')
		sendmail('cjsima@gmail.com', 'belladonnasanctuary@gmail.com', name)
		...
		# 	return
			try:
				...
				existing_pdf = PdfFileReader(file("AmericanPriestessCert.pdf", "rb"))
				...
				self.response.headers['Content-Disposition'] = 'attachment; filename=AmericanPriestessCert.pdf'
